...
const Cart = () => {
  const { cart, setIsCartOpen, setCart } = useContext(MyStore);

  const total = cart.reduce((sum, item) => sum + item.price, 0);

  const deletedItem = (id) => {
    let deletedProduct = cart.filter((val) => val.id !== id);
    ...
    <div className="w-105 h-screen bg-[#111111] border-l border-gray-700 flex flex-col">
      ...
      {cart.length === 0 ? (
        <div className="flex-1 flex flex-col items-center justify-center px-8">
          ...
          {/* Heading */}
          <h2 className="text-white text-4xl font-bold mt-8">Cart is empty</h2>
          ...
          <button
            ...
            className="mt-10 bg-lime-400 hover:bg-lime-500 text-black font-semibold text-lg px-10 py-4 rounded-2xl transition-all duration-300"
          >
            Browse Products
          </button>
        </div>
              ...
              <div className="flex-1">
                ...
                <div className="flex items-center justify-between mt-4">
                  <div className="flex items-center gap-4">
                    <button className="border border-gray-700 rounded-lg w-9 h-9 flex items-center justify-center text-white">
                      <Minus size={15} />
                    </button>
      ...
      <div className="border-t border-gray-700 p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <h1 className="text-white text-4xl font-bold">${total.toFixed(2)}</h1>
        </div>
        ...
        <button className="w-full mt-4 text-gray-500 hover:text-white transition">
          Clear cart
        </button>
      </div>
    </div>
